# VaultForge — Pre-Mainnet Checklist
...
| 3 | ReentrancyGuard on VaultFactory.deployVault | **DONE** | Defense-in-depth — calls `initialize()` on clone |
...
| # | Item | Status | Notes |
|---|------|--------|-------|
| 18 | Rate limiting on all public endpoints | **DONE** | slowapi — 10/min vault, 30/min health, 5/min optimize |
...
| 22 | Privy JWT verification on protected routes | **DEFERRED → Mainnet** | Backend validates wallet address format (regex). Full Privy JWT verification requires Privy secret in production. Testnet demo uses address-only auth |
| 23 | Integration tests pass | **DONE** | test_integration.py — create → health flow |

## Frontend (Next.js)

| # | Item | Status | Notes |
|---|------|--------|-------|
| 24 | Supabase service key NOT in any frontend file | **DONE** | Grep confirmed — only `NEXT_PUBLIC_SUPABASE_ANON_KEY` used |
...
| 28 | CSP headers (Content-Security-Policy) | **DEFERRED → Mainnet** | Add strict CSP in next.config.ts before production deploy. Testnet demo runs on localhost |
...
| 33 | Database backup strategy | **DEFERRED → Mainnet** | Supabase Pro plan includes point-in-time recovery. Will enable before mainnet user data |
...
| 41 | Monitoring / alerting (contract events) | **DEFERRED → Mainnet** | OpenZeppelin Defender or custom event indexer. Not needed for testnet demo with known deployer |
| 42 | Error tracking (Sentry / equivalent) | **DEFERRED → Mainnet** | Sentry integration for frontend + backend. Testnet uses console logging |

## Integration Tests

| # | Item | Status | Notes |
|---|------|--------|-------|
| 43 | Foundry: full lifecycle (deploy → deposit → borrow → repay → withdraw) | **DONE** | IntegrationTest.t.sol |
...
| 47 | Backend: create vault → health check API flow | **DONE** | test_integration.py |
| 48 | Frontend ↔ Backend ↔ Chain e2e (Cypress/Playwright) | **DEFERRED → Mainnet** | Playwright e2e tests planned. Current coverage: Foundry e2e (5 lifecycle tests) + pytest e2e (3 API flow tests) + `npm run build` TypeScript check |
