...
**[opBNBScan Contracts](https://testnet.opbnbscan.com)** |
...
flowchart TB
    ...
    subgraph Backend["Backend — FastAPI (Docker)"]
        API[REST Endpoints]
        Optimizer[LTV Optimizer — SciPy + PuLP]
        Prover[ZK Proof Orchestrator — snarkjs]
...
| **ZK Proofs** | Circom 2.2.3 + snarkjs (Groth16) | Constant-size proofs, cheap on-chain verification |
| **Frontend** | Next.js 16 + React 19 + TypeScript 5 | App Router, React Compiler, server components |
| **Wallet Auth** | Privy + Account Abstraction | Embedded wallets, social login, gasless UX |
| **Web3 Hooks** | wagmi 3.5 + viem 2.46 | Type-safe contract reads/writes, multicall |
...
| **Optimization** | SciPy + PuLP | Linear programming for LTV optimization |
| **Database** | Supabase Postgres + RLS | Row-level security, real-time subscriptions |
| **Cache** | Redis 7 | Sub-ms price feed caching |
...
git clone https://github.com/vaultforge/vaultforge.git && cd vaultforge
...
```bash
# Contracts
cd contracts && forge build && forge test   # 38 tests
...
| `NEXT_PUBLIC_VAULT_FACTORY_ADDRESS` | Deployed VaultFactory proxy address | `0x1234...` |
| `NEXT_PUBLIC_VAULT_IMPLEMENTATION_ADDRESS` | Vault implementation address | `0x5678...` |
| `NEXT_PUBLIC_ZK_VERIFIER_ADDRESS` | Deployed ZKVerifier address | `0x9abc...` |
| `NEXT_PUBLIC_AGENT_REGISTRY_ADDRESS` | Deployed AgentRegistry address | `0xdef0...` |
| `NEXT_PUBLIC_RELAYER_ADDRESS` | Meta-tx relayer address | `0xaaaa...` |
| `NEXT_PUBLIC_PRIVY_APP_ID` | Privy App ID for wallet auth | `cmm4pm...` |
| `PRIVY_APP_SECRET` | Privy secret for JWT verification | `privy_app_secret_...` |
...
| `SUPABASE_DB_URL` | Direct Postgres connection string | `postgresql://postgres:pw@host:5432/db` |
...
| `SNARKJS_PATH` | Path to snarkjs CLI binary | `./zk-circuits/node_modules/.bin/snarkjs` |
| `CHAINLINK_BNB_USD_FEED` | Chainlink BNB/USD price feed address | `0xbbbb...` |
...
| **ZKVerifier** | `0x849Ca487D5DeD85c93fc3600338a419B100833a8` | [View](https://opbnb-testnet.bscscan.com/address/0x849Ca487D5DeD85c93fc3600338a419B100833a8) | ✅ |
...
|---|---|---|
| **opBNB L2** | All contracts deployed on opBNB testnet (chain 5611) | [Deployment tx](https://opbnb-testnet.bscscan.com/tx/0x14478a9a03e7fc600ad7c59dcd9036abbe37f21ad54ad0befc0e4c4ac00277bb) |
| **EIP-1167 Minimal Proxies** | VaultFactory deploys gas-efficient vault clones | Clone cost: 207,659 gas ≈ **$0.0000002 USD** |
| **BNB AI Agent Framework** | AgentRegistry.sol — agents stake tBNB, execute vault actions, earn fees, get slashed | [Contract](https://opbnb-testnet.bscscan.com/address/0xD5932aF5c315C0A1fD9D486E0f58b7C210866ADF) |
| **ZK Privacy (Groth16)** | ZKVerifier.sol — on-chain proof verification with nonce replay prevention | [Contract](https://opbnb-testnet.bscscan.com/address/0x849Ca487D5DeD85c93fc3600338a419B100833a8) |
| **Optimistic Oracle** | LTVOracle.sol — off-chain optimizer submits LTV with 1h challenge window | [Contract](https://opbnb-testnet.bscscan.com/address/0x4B6171fA771fdA1F86445a5C06b0d5dA11875BC4) |
...
5 production contracts, **38 passing tests** (31 unit + 5 integration + 2 utility):
...
| **ZKVerifier.sol** | Groth16 proof verification | Proof replay prevention (hash + per-vault nonce) |
...
3 Circom 2.2.3 circuits with Groth16 proving system:
...
Proofs are generated client-side via snarkjs, verified on-chain by `ZKVerifier.sol`. Trusted setup uses `powersOfTau28_hez_final_14.ptau` (Hermez ceremony).
...
|---|---|---|---|
| **Phase 1** — Hack MVP | opBNB testnet live | Feb 2026 | ✅ 5 contracts deployed, 3 ZK circuits proven, AI agent demo, 38 tests pass |
...
│   └── tests/          34 pytest tests (98% coverage)
...
|---|---|---|---|
| **Smart Contracts** | Foundry (forge test) | 38 passing | `cd contracts && forge test -vvv` |
| **Backend API** | pytest + pytest-cov | 34 passing (98% cov) | `cd backend && uv run pytest --cov` |
...
| **Integration (Solidity)** | Foundry e2e | 5 lifecycle tests | Full deploy→deposit→borrow→repay→withdraw |
| **Integration (Python)** | pytest e2e | 3 API flow tests | Vault create→health check→optimize |
